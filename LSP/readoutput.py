...
#fn = "output_higgsinoLSP_100_slep.pkl"
fn = "msl500_tb3_ma_2000.pkl"
fp = open(fn)

temp = fn.split(".")[0]
outfnbase = "plot_" + temp
...
for i in range(n):
    ...
    msnu[i]  = masslist[i][1000012]['mass']
    ...
    M1[i]    = inputlist[i][1]['value']
    M2[i]    = inputlist[i][2]['value']
...
stop_decays = ["~chi_10 e", "~chi_1- e", "~chi_20 e",
               "~t_1    Z", "~t_1    h", "~b_1    W"]

sbottom_decays = ["~chi_10 nu", "~chi_1+ e", "~chi_20 nu",
                  "~t_1    W", "blah", "blah1"]
...
def makeplots(titlebase):
    ...
    if titlebase == 'selecL':
        ...
        stoplist = snulist[0:halfind]
        important_decays = sbottom_decays
        mass_array = msnu[0:halfind]
    elif titlebase == 'selecL_high':
        stoplist = selecLlist[0:halfind]
        important_decays = stop_decays
        mass_array = mq[0:halfind]
        xlabel = 'M_decouple'
    elif titlebase == 'selecR_high':
        stoplist = selecRlist[halfind:]
        important_decays = stop_decays
        mass_array = mq[halfind:]
        xlabel = 'M_decouple'
    elif titlebase == 'snu_high':
        stoplist = snulist[0:halfind]
        important_decays = sbottom_decays
        mass_array = mq[0:halfind]
        xlabel = 'M_decouple'
    elif titlebase == 'selecL_M1':
        stoplist = selecLlist[0:halfind]
        important_decays = stop_decays
        mass_array = M1[0:halfind]
        xlabel = 'M1'
    elif titlebase == 'selecR_M1':
        stoplist = selecRlist[halfind:]
        important_decays = stop_decays
        mass_array = M1[halfind:]
        xlabel = 'M1'
    elif titlebase == 'snu_M1':
        stoplist = snulist[0:halfind]
        important_decays = sbottom_decays
        mass_array = M1[0:halfind]
        xlabel = 'M1'
    elif titlebase == 'selecL_M2':
        stoplist = selecLlist[0:halfind]
        important_decays = stop_decays
        mass_array = M2[0:halfind]
        xlabel = 'M2'
    elif titlebase == 'selecR_M2':
        stoplist = selecRlist[halfind:]
        important_decays = stop_decays
        mass_array = M2[halfind:]
        xlabel = 'M2'
    elif titlebase == 'snu_M2':
        stoplist = snulist[0:halfind]
        important_decays = stop_decays
        mass_array = M2[0:halfind]
        xlabel = 'M2'
        ...
        for k in range(len(keylist)):
            ...
            if (d in keylist[k]):
                ...
                if allpositive(yarray[k][:]):
                    ...
                    myList.append((keylist[k], xarray, yarray[k][:]))
                else:
                    py.semilogy(xarray, yarray[k][:],s,label=keylist[k])
    ...
    for i in indexlist:
        if allpositive(yarray[i][:]):
            if len(keylist) > 7:
                ...
                py.semilogy(xarray, yarray[i][:],'-o',label=keylist[i])
                myList.append((keylist[i], xarray, yarray[i][:]))
...
for i in range(len(myList)):
    myArray = array(myList[i][1:])
    myBR    = myList[i][0]
    savetxt(temp + "_" + myBR + ".dat", myArray)
